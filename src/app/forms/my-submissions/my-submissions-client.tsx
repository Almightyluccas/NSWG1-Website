  ...
  const renderAnswer = (answer: FormSubmissionAnswer) => {
    ...
    switch (question_type) {
      case "checkboxes":
        ...
        return <p className="italic text-muted-foreground">No options selected</p>
      ...
      default:
        ...
        if (answer_json) {
          return <p className="whitespace-pre-wrap">{JSON.stringify(answer_json, null, 2)}</p>
        }
        return <p className="italic text-muted-foreground">No answer provided</p>
    }
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {submissions.map((submission) => (
            <Card key={submission.id} className="flex flex-col justify-between">
              <CardHeader>
                <CardTitle className="text-lg">{submission.form_title || "Unknown Form"}</CardTitle>
                <CardDescription>
                  Submitted on{" "}
                  {submission.submitted_at ? format(new Date(submission.submitted_at), "PPP") : "Unknown Date"}
                </CardDescription>
              </CardHeader>
              ...
              <CardFooter>
                <Button onClick={() => handleViewDetailsClick(submission)} className="w-full">
                  View Details
                </Button>
              </CardFooter>
            </Card>
      ...
      {selectedSubmission && (
        <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
          <DialogContent className="sm:max-w-[800px] h-[80vh] flex flex-col">
            <DialogHeader>
              <DialogTitle>{selectedSubmission.form_title || "Form Submission Details"}</DialogTitle>
              <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                <DialogDescription className="inline">
                  Submitted by {selectedSubmission.user_name || selectedSubmission.user_email || "Unknown User"} on{" "}
                ...
                {selectedSubmission.reviewed_at && (
                  <DialogDescription className="inline">
                    Reviewed on {format(new Date(selectedSubmission.reviewed_at), "PPP p")}.
                  </DialogDescription>
                )}
                <Badge className={getStatusColorClass(selectedSubmission.status)}>
            ...
            {selectedSubmission.notes && (
              <div className="py-4 px-6 border-b border-t bg-muted/50 rounded-md mx-6 mt-4">
                ...
                <p className="text-muted-foreground text-sm whitespace-pre-wrap">{selectedSubmission.notes}</p>
              </div>
            )}
                ...
                {selectedSubmission.answers && selectedSubmission.answers.length > 0 ? (
                  selectedSubmission.answers.map((answer, index) => (
                    <div key={answer.id} className="space-y-2">
                      ...
                      <div className="text-foreground text-sm">{renderAnswer(answer)}</div>
                      {index < selectedSubmission.answers!.length - 1 && <Separator className="my-4" />}
                    </div>
                  ))
                ) : (
                  <p className="text-center text-muted-foreground">No answers found for this submission.</p>
                )}
              </div>
            </ScrollArea>
          </DialogContent>
