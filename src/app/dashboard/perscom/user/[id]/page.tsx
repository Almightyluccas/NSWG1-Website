...
import { UserProfile } from "@/app/dashboard/perscom/user/[id]/user-profile.client";
...
export default async function DashboardUserProfilePage({
  ...
  const user = allUsers.find((profile) => profile.id.toString() === id);
  if (user) {
    ...
    ? allQualifications
        .filter((qualification) =>
          user.qualification_records?.some(
            (record) => record.qualification_id === qualification.id
          )
        )
        .map((qualification) => ({
          id: qualification.id,
  ...
  const assignmentHistory = user?.assignment_records
    ? user.assignment_records.map((record) => {
        ...
        )?.position || { name: `Unknown Position (ID: ${record.position_id})` };
