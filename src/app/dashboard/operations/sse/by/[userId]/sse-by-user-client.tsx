    ...
    async function load() {
      try {
        const res = await fetch(
          `/api/sse?uploadedBy=${encodeURIComponent(userId)}`
        );
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((item) => (
            <div
              key={item.id}
              className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950/50 p-3 space-y-3"
            >
