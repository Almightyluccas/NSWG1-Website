...
function getCategoryIcon(typeKey: string) {
  ...
  return (
    <span className={cat.iconColor}>{icon ?? <Box className="h-4 w-4" />}</span>
  );
}
...
export function SseLibraryClient({ userRoles = [] }: { userRoles?: string[] }) {
  ...
  const [sortBy, setSortBy] = useState<
    "DATE_DESC" | "DATE_ASC" | "OP_ASC" | "OP_DESC"
  >("DATE_DESC");
  ...
  const filteredItems = items
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "OP_DESC")
        return String(b.campaign_id).localeCompare(String(a.campaign_id));
      return 0;
    });
        ...
        {paginatedItems.length === 0 ? (
          <div className="py-12 text-center bg-white dark:bg-zinc-900/40 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-lg">
            ...
            <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest">
              NO REGISTRIES MATCH QUERY PARAMETERS
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {paginatedItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="group relative md:grid md:grid-cols-12 gap-3 px-5 py-3 items-center bg-zinc-50 dark:bg-zinc-950/60 hover:bg-zinc-100 dark:hover:bg-zinc-900 border border-zinc-200 dark:border-zinc-800/80 hover:border-zinc-300 dark:hover:border-zinc-600 transition-all rounded-lg"
                >
                  {/* Glowing Status Indicator Bar */}
                  <div
                    className={`absolute left-0 top-0 bottom-0 w-1 opacity-70 group-hover:opacity-100 transition-opacity ${clr.strip}`}
                  />
                  ...
                  <div className="col-span-4 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <h4 className="text-[13px] font-black text-zinc-900 dark:text-zinc-100 truncate tracking-wide">
                        {item.name || "UNNAMED ASSET"}
                      </h4>
                      <p className="text-[9px] font-mono font-medium text-accent/80 mt-0.5 uppercase tracking-widest">
                        <span className="text-zinc-500 mr-1">ID:</span>SYS-X
                  ...
                  <div className="col-span-2 mt-3 md:mt-0">
                    <p className="text-[11px] font-bold text-zinc-700 dark:text-zinc-300 truncate uppercase tracking-wider">
                      {item.campaign_name || `OP-${item.campaign_id}`}
                    </p>
                  </div>
                  ...
                  <div className="col-span-2 mt-2 md:mt-0">
                    <Badge
                      variant="outline"
                      className={`text-[9px] uppercase font-black tracking-widest px-2 py-0 rounded-lg ${clr.border} ${clr.text} ${clr.bg}`}
                    >
                      {clr.label}
                    </Badge>
                  </div>
                  ...
                  <div className="col-span-2 flex flex-col items-start justify-center mt-2 md:mt-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      ...
                      <span
                        className={`text-[9px] font-black uppercase tracking-widest ${statusCfg.text}`}
                      >
                        {item.status}
                      </span>
                    </div>
                  ...
                  <div className="col-span-2 flex md:justify-end gap-1.5 mt-3 md:mt-0">
                    ...
                    <Link
                      href={`/dashboard/operations/sse/${item.id}`}
                      className="flex-1 md:flex-none"
                    >
                      <Button
                        size="sm"
                        variant="outline"
                        className="w-full h-7 rounded-md text-[9px] font-black uppercase tracking-widest border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-accent hover:text-black hover:border-accent transition-all bg-white dark:bg-zinc-900 group-hover:border-zinc-300 dark:group-hover:border-zinc-500"
                      >
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-zinc-200 dark:border-zinc-800/80 pt-4 mt-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-[10px] text-zinc-700 dark:text-zinc-300 font-bold px-2">
