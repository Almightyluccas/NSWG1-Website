  ...
  if (!record) {
    return (
      <div className="py-20 text-center bg-zinc-50 dark:bg-zinc-950/60 border border-zinc-200 dark:border-zinc-800 rounded-lg mt-6">
        ...
        <p className="text-sm font-mono text-zinc-600 mt-2">The requested payload could not be located in the registry.</p>
      </div>
    );
  }
      ...
      {showNotification && (
        <div className="bg-emerald-500/10 border border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.15)] rounded-lg p-4 flex items-center gap-4 animate-in fade-in slide-in-from-top-4 relative overflow-hidden">
          ...
          <div className="flex-1">
            <p className="text-xs font-black text-emerald-400 uppercase tracking-widest">NETWORK AUTHENTICATION: SUCCESS</p>
            <p className="text-[10px] uppercase font-mono tracking-widest text-emerald-300/80 mt-1">Classification and status applied. Automated notification dispatched to source terminal // {record.uploaded_by}.</p>
          </div>
        </div>
      )}
            ...
            <CardHeader className="border-b border-zinc-200 dark:border-zinc-800/60 p-5 relative z-10">
              <div className="flex justify-between items-start gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-zinc-900 dark:text-white uppercase tracking-widest drop-shadow-sm">{record.name}</h2>
                ...
                <div className="flex flex-col items-end gap-2 text-right">
                   <Badge variant="outline" className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-lg border shadow-sm ${clr.border} ${clr.text} ${clr.bg}`}>
                     {clr.label}
                   </Badge>
                   <p className="text-[9px] font-mono font-medium text-zinc-500 tracking-widest">SYS.ID: {record.id}</p>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-0 relative z-10">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 bg-white dark:bg-zinc-900/40 divide-y lg:divide-y-0 lg:divide-x divide-zinc-200 dark:divide-zinc-800/60 border-b border-zinc-200 dark:border-zinc-800/60">
                <div className="p-4 space-y-1">
                  ...
                  <p className={`text-xs font-black uppercase tracking-widest ${statusCfg.text}`}>{status}</p>
                </div>
                <div className="p-4 space-y-1">
                  <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-1.5">
                ...
                <div className="p-4 space-y-1">
                  ...
                  <p className="text-xs text-zinc-800 dark:text-zinc-200 font-bold tracking-wider">{record.type}</p>
                </div>
                <div className="p-4 space-y-1">
                   <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-1.5">
                    <Zap className="h-3 w-3" /> Timestamp
                  </p>
                  <p className="text-xs font-mono text-zinc-800 dark:text-zinc-200 tracking-wider font-semibold">{new Date(record.created_at).toLocaleDateString()}</p>
                </div>
              </div>
              ...
              <div className="p-5 md:p-6 pb-2">
                 {record.image_url && (
                   <div className="mb-5 w-full aspect-video rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-800/60 shadow-inner group relative">
                      ...
                      <img src={record.image_url} alt={record.name || "SSE Visual"} className="w-full h-full object-cover" />
                      <div className="absolute bottom-2 right-2 bg-white/90 dark:bg-zinc-900/90 text-zinc-700 dark:text-zinc-300 text-[9px] font-mono px-2 py-0.5 border border-zinc-200 dark:border-zinc-700/80 rounded-lg z-20">VISUAL.LINK // VALID</div>
                   </div>
                 )}
              ...
              <div className="p-5 md:p-6 pt-3">
                 <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">Extracted Cipher Artifacts</h3>
                 <div className="flex items-center justify-between p-3 rounded-lg border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-600 transition-colors bg-white dark:bg-zinc-900/40 group">
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          <p className="text-xs font-bold text-zinc-800 dark:text-zinc-200 tracking-wider">PAYLOAD_EXTRACT_{String(record.id).padStart(3, '0')}.dat</p>
                          <p className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest mt-0.5">4.2 MB • ENCRYPTED_ARCHIVE</p>
                       </div>
                    </div>
                    <Button variant="outline" size="sm" className="h-8 w-8 p-0 border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-accent hover:text-black hover:border-accent transition-all shadow-sm rounded-lg">
              ...
              <div className="space-y-6 flex flex-col h-full">
                 <div className="bg-accent/10 border border-accent/20 p-3 rounded-lg flex items-start gap-3 shadow-inner">
                    ...
                    <p className="text-[10px] text-zinc-700 dark:text-zinc-300 leading-relaxed font-mono uppercase tracking-widest">
                      Authorization Confirmed: <span className="text-accent font-black">Intelligence Officer</span>. Access modification permits granted for target payload.
                    </p>
                 </div>

                 <div className="space-y-4">
                     <div className="space-y-2">
                        <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Target Classification Filter</label>
                        <select 
                     ...
                     <div className="space-y-2">
                        <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Operational Sequence State</label>
                        <select 
                          value={status}
            ...
            {uploaderItems.map((item) => (
              ...
              >
                <div className="h-32 w-full rounded-md overflow-hidden border border-zinc-200 dark:border-zinc-700">
                  {item.image_url ? (
                    <img src={item.image_url} alt={item.name || "SSE"} className="w-full h-full object-cover" />
                  ) : (
