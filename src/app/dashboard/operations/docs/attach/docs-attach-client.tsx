...
export function DocsAttachClient({
  campaignId,
  missionId,
}: {
  campaignId: string;
  ...
  useEffect(() => {
    async function load() {
      const res = await fetch(`/api/docs?missionId=${missionId}`);
  ...
  return (
    <RepositoryPicker
      ...
      fetchUrl={`/api/docs?campaignId=${campaignId}&scope=repository`}
      attachUrl={(id) => `/api/docs/${id}/attach`}
      missionId={missionId}
