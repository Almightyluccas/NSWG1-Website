...
export async function createRecurringTraining(
  data: CreateRecurringTrainingData
): Promise<string> {
...
export async function getRecurringTrainings(): Promise<
  RecurringTrainingWithStats[]
> {
  const session = await getServerSession(authOptions);
  if (!session?.user?.roles.includes("admin")) {
...
export async function updateRecurringTraining(
  id: string,
  ...
  for (const recurring of recurringTrainings) {
    try {
      ...
      for (let weekOffset = 1; weekOffset <= 3; weekOffset++) {
        ...
        const createdTraining = await database.get.trainingRecordByDetails(
          recurring.name,
          finalDate,
          recurring.time
        );
...
export async function getRecurringTrainingInstances(
