...
export default async function AdminDashboard() {
  ...
  const pendingApplications = await perscom.get
    .applications()
    .then((applications) => {
      return applications.filter((application) => {
        return (
          Array.isArray(application.statuses) &&
          application.statuses.length === 0
        );
      }).length;
    });
  const totalUsers = await perscom.get.users().then((users) => {
    return users.length;
  });
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-zinc-400">
            Welcome to the NSWG1 admin dashboard.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <AdminStatCard
            ...
            trend={{
              value: newUsersThisMonth.percentChange,
              isPositive: newUsersThisMonth.isPositive,
            }}
          />
          <AdminStatCard
            ...
            <div className="space-y-3">
              ...
              >
                <div className="font-medium">
                  Review Enlistment Applications
                </div>
              ...
              >
                ...
                <div className="text-sm text-gray-500 dark:text-zinc-400">
                  Configure unit structure and assignments
                </div>
              </a>
            </div>
          </div>
