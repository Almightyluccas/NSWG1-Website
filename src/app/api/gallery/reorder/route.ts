...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (
      ...
      orderedIds.some((x) => typeof x !== "number" && typeof x !== "string")
    ) {
      return NextResponse.json(
        { error: "orderedIds must be an array of numeric ids" },
        { status: 400 }
      );
    }

    const ids = orderedIds.map((x) =>
      typeof x === "number" ? x : parseInt(String(x), 10)
    );
    if (ids.some((n) => Number.isNaN(n))) {
      return NextResponse.json(
        { error: "Invalid id in orderedIds" },
        { status: 400 }
      );
    }
