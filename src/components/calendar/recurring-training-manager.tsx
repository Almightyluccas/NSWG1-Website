...
import { ChevronDown, ChevronUp, Calendar, Clock, MapPin, User, Users, Trash2, Play, RefreshCw } from "lucide-react"
...
export function RecurringTrainingManager() {
  const [recurringTrainings, setRecurringTrainings] = useState<RecurringTraining[]>([])
  ...
  const handleProcessTrainings = async () => {
    ...
    try {
      ...
      if (created > 0) {
        toast.success(`Created ${created} training session${created > 1 ? "s" : ""}`)
      }
      if (skipped > 0) {
        toast.warning(`Skipped ${skipped} training${skipped > 1 ? "s" : ""} due to conflicts`)
      }
      if (errors > 0) {
        toast.error(`Failed to process ${errors} training${errors > 1 ? "s" : ""}`)
      }
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Manage weekly recurring training sessions</p>
            </div>
        ...
        <CollapsibleContent>
          <CardContent>
            <div className="space-y-4">
              ...
              {loading ? (
                <div className="text-center py-4">Loading recurring trainings...</div>
              ) : recurringTrainings.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  <Calendar className="h-12 w-12 mx-auto mb-4 opacity-50" />
                  <p>No recurring trainings configured</p>
                  <p className="text-sm">Create one to automatically schedule weekly training sessions</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {recurringTrainings.map((training) => (
                    <Card key={training.id} className="border-l-4 border-l-accent">
                          ...
                          <div className="flex items-center gap-2 ml-4">
                            <div className="flex items-center gap-2">
                              <Switch
                                ...
                                onCheckedChange={(checked) => handleToggleActive(training.id, checked)}
                              />
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  <AlertDialogTitle>Delete Recurring Training</AlertDialogTitle>
                                  <AlertDialogDescription>
                                    Are you sure you want to delete "{training.name}"? This will also delete all
                                    associated training instances. This action cannot be undone.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <ul className="space-y-1 text-muted-foreground">
                      <li>• Training sessions are automatically created 1 week in advance</li>
                      <li>• System checks for mission conflicts and reschedules if needed</li>
                      <li>• Only active recurring trainings will create new sessions</li>
                      <li>• Click "Process Now" to manually check for new sessions to create</li>
                    </ul>
                  </div>
                </div>
              </div>
