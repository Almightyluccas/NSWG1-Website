  ...
  return (
    <div className="relative w-full aspect-[5/3] bg-zinc-950/80 rounded-sm overflow-hidden border border-zinc-800/80 group">
      ...
      >
        ...
        <path d="M120 180 Q150 150 180 160 T250 190 T280 250 T220 300 T180 350 T150 450 T130 500 T100 480 T80 400 T70 300 T90 220 Z" /> {/* Americas rough shape */}
        <path d="M450 150 Q500 120 540 140 T580 180 T650 190 T700 150 T750 120 T800 160 T850 200 T900 250 T880 300 T850 350 T750 400 T650 380 T550 450 T500 480 T460 380 T420 350 T400 300 T420 200 Z" /> {/* Eurasia/Africa rough shape */}
        <path d="M750 450 Q800 420 850 450 T900 500 T850 550 T780 520 Z" /> {/* Oceania rough shape */}
      </svg>

      {/* Crosshairs & Target Info lines */}
      <div className="absolute top-0 bottom-0 w-px bg-accent/20 z-10 pointer-events-none" style={{ left: `${x}%` }} />
      <div className="absolute left-0 right-0 h-px bg-accent/20 z-10 pointer-events-none" style={{ top: `${y}%` }} />
      ...
      >
        ...
        <div className="absolute left-6 top-1/2 -translate-y-1/2 bg-zinc-950/90 border border-accent/40 px-2 py-1 rounded-sm opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none backdrop-blur-md">
           <p className="text-[9px] font-mono text-accent uppercase">TGT LOCK {lat.toFixed(2)}, {lng.toFixed(2)}</p>
        </div>
      </div>
      ...
      <div className="absolute bottom-2 left-2 z-20 bg-zinc-950/80 border border-zinc-800 px-2 py-1 rounded-sm backdrop-blur-md">
        <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest flex items-center gap-2">
          <span className="text-accent/60">LAT</span> {Math.abs(lat).toFixed(4)}° {lat >= 0 ? 'N' : 'S'} 
          <span className="text-zinc-700">|</span> 
          <span className="text-accent/60">LNG</span> {Math.abs(lng).toFixed(4)}° {lng >= 0 ? 'E' : 'W'}
        </p>
      </div>
      ...
      <div className="absolute top-2 right-2 z-20 flex gap-1">
         ...
         <div className="text-[8px] font-mono text-accent/60 tracking-widest">SAT-LINK ACTIVE</div>
      </div>
    </div>
  );
}
