...
const getPresignedUrl = async (
  ...
  if (!presignedUrlResponse.ok) {
    ...
    const errorMessage =
      errorData?.error || "Failed to get a secure upload URL.";
...
const saveUploadDetails = async (
  key: string,
  uploadType: UploadType,
  metadata?: GalleryItem | DocumentMetaData
) => {
  ...
  if (!saveKeyResponse.ok)
    throw new Error("Failed to save upload details to database.");
};
